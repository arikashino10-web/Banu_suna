...
function extractAssistantText(data) {
  ...
  const raw = textFromValue(data?.raw);
  if (raw && !/^\s*<!doctype html[\s>]/i.test(raw) && !/^\s*<html[\s>]/i.test(raw)) return raw;
  return null;
}
...
async function askAI(systemPrompt, userMessage) {
  ...
  try {
    const geminiText = await gemini.askGemini(userMessage);
...
For casual questions, answer in 1-3 short sentences. Do not output raw HTML, CSS, JavaScript, or JSON unless the user explicitly asks for code. When code is requested, explain it briefly and use a fenced code block; never claim that code was deployed or edited.
...
async function handleInput(context, rawInput, { starting = false } = {}) {
  ...
  const response = result
    ? `${result.text}\n\n— ${result.provider}`
    : "⚠️ AI service সাময়িকভাবে উত্তর দিতে পারছে না। কিছুক্ষণ পরে আবার চেষ্টা করুন।";
  addMessage(threadID, "GroupAI", response);
  return response;
}

...
const statsPath = path.join(__dirname, 'cache', 'command_stats_0up.json');
...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    description: "See bot uptime and most used commands in a large neon System Overview HUD (no-prefix: just type 0up)",
    category: "system"
  },

  // No-prefix trigger: this fires on EVERY message. We check if the
  // message text is exactly "0up" (case-insensitive) and run the HUD.
  onChat: async function ({ api, event }) {
    const body = (event.body || '').trim();
    if (!body) return;

    // Only trigger on an exact "0up" message (ignoring case/extra spaces)
    if (body.toLowerCase() !== '0up') return;
...
⚡ Powered by JABED`;
    ...
    try {
      const cachePath = path.join(__dirname, 'cache', `zeroup_hud.png`);
  ...
  // Keeping onStart too, in case the bot loader requires it and/or
  // someone still calls it with the prefix (prefix + 0up).
